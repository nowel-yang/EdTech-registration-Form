...
class PTA_SUS_Public {
    ...
    private $time_start_header; # added
    private $time_end_header; # added
    ...
    public function set_up_filters() {
        ...
        $this->task_item_header = apply_filters( 'pta_sus_public_output', __('Presenter', 'pta_volunteer_sus'), 'task_item_header' );
     ...
     */
    public function display_sheet($atts, $sheets) {
        ...
        $start_time_header = apply_filters( 'pta_sus_public_output', __('Date', 'pta_volunteer_sus'), 'start_time_header' );
        ...
        extract( shortcode_atts( array(
            'id' => false,
            'date' => false,
            'list_title' => __('Currently Offered Workshops', 'pta_volunteer_sus'),
        ...
        if('' == $date) $date = false;
        if('' == $list_title) $list_title = __('Currently Offered Workshops', 'pta_volunteer_sus');
        ...
        if ($id === false) {
            ...
            if (empty($sheets)) {
                $return .= '<p>'.apply_filters( 'pta_sus_public_output', __('No workshops are currently being offered. If you need help with something, please email us at <a href="mailto:edtech@langara.bc.ca">edtech@langara.bc.ca</a> or call 604-323-5403', 'pta_volunteer_sus'), 'no_sheets_message' ).'</p>';
            } else {
                ...
                $return .=     '<th class="column-date">'.esc_html( $start_time_header ).'</th>
                                <th class="column-time-start">'.esc_html( $time_start_header ).'</th>
                                <th class="column-time-end">'.esc_html( $time_end_header ).'</th>
                        ...
                        foreach ($sheets AS $sheet) {
                            ...
                            $view_signup_text = apply_filters( 'pta_sus_public_output', __('Register', 'pta_volunteer_sus'), 'view_and_signup_link_text' );
		    ...
		    $view_all_text = apply_filters('pta_sus_public_output', __('View all Workshop Registrations', 'pta_volunteer_sus'), 'view_all_sheets_link_text');
                    $return .= '
			    ...
			    if (!$this->submitted || $this->success || $this->err) {
                    ...
                    if($future_dates) {
                        ...
                        $open_spots = ($this->data->get_sheet_total_spots($sheet->id) - $this->data->get_sheet_signup_count($sheet->id));
                        if ($open_spots > 0) {
                            $return .= '<h3>'.apply_filters( 'pta_sus_public_output', __('Register Now', 'pta_volunteer_sus'), 'sign_up_below' ).'</h3>';               
                        } else {
                    ...
                    foreach ($tasks AS $task) {
                        ...
                        foreach ($signups AS $signup) {
                            ...
                            if($this->main_options['hide_volunteer_names']) {
                                ...
                                $display_signup = esc_html($signup->firstname).' '.esc_html($signup->lastname);
                            }
	...
	public function display_signup_form($task_id, $date) {	
        ...
        // Originally is confirm_email_lable
        // $validate_email_label = apply_filters( 'pta_sus_public_output', __('Confirm E-mail', 'pta_volunteer_sus'), 'confirm_email_label' );
        $department_label = apply_filters( 'pta_sus_public_output', __('Department', 'pta_volunteer_sus'), 'department_label' );
        ...
        if ( is_user_logged_in() ) {
			...
			<form name="pta_sus_signup_form" method="post" action="">
				...
				<p>
					<label for="signup_department">'.$department_label.'</label>
					<input type="text" id="signup_department" name="signup_department" value="'. esc_attr($current_user->user_department) .'" />
				</p>
            ...
            if (false == $this->main_options['disable_signup_login_notice']) {
                $form .= '<p>'.apply_filters( 'pta_sus_public_output', __('If you have a Langara iWeb account, it is recommended that you <strong>login before you sign up</strong> so that you can view and edit all your signups.', 'pta_volunteer_sus'), 'signup_login_notice' ).'</p>';
            }
            $form .= '
			<form name="pta_sus_signup_form" method="post" action="">
				<p>
				...
				<p>
					<label for="signup_department">'.$department_label.'</label>
					<input type="text" id="signup_department" name="signup_department" value="'.((isset($_POST['signup_lastname'])) ? esc_attr($_POST['signup_department']) : '').'" />
				</p>
				<p>
